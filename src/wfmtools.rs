...
pub mod wfm_tools {
    ...
    impl WFMHeader {
        fn parse_header(&mut self, header: &[u8]) {
            ...
            self.num_impl_dim =
                u32::from_le_bytes(nimpd.try_into().expect("Conversion Failed")) as u8;
            self.num_expl_dim =
                u32::from_le_bytes(nexpd.try_into().expect("Conversion Failed")) as u8;
            ...
            self.record_type =
                u32::from_le_bytes(rtype.try_into().expect("Record Type import failed")) as u8;
            self.expl_dim_type =
                u32::from_le_bytes(extype.try_into().expect("Explicit Dimension failed")) as u8;
            ...
            if isff == 1 {
                ...
                self.num_fastframes = u32::from_le_bytes(
                    numframes
                        .try_into()
                        .expect("Error parsing the number of FastFrames in the file."),
                ) + 1;
            } else if isff == 0 {
            ...
            self.voltage_scale =
                f64::from_le_bytes(vscale.try_into().expect("Problem parsing voltage scale"));
            self.voltage_offset =
                f64::from_le_bytes(voffset.try_into().expect("Problem parsing voltage offset"));
            self.acq_time_scale =
                f64::from_le_bytes(tscale.try_into().expect("Problem parsing time step"));
            self.acq_time_start = f64::from_le_bytes(
                tstart
                    .try_into()
            ...
            self.precharge_offset =
                u32::from_le_bytes(preoff.try_into().expect("Precharge conversion failed")) as u16;
            self.postcharge_offset =
                u32::from_le_bytes(postoff.try_into().expect("Postcharge conversion failed"))
                    as u16;
            self.usable_record_length = self.postcharge_offset - self.precharge_offset;
    ...
    impl WFMFile {
        pub fn new() -> WFMFile {
            let wfm_object = WFMFile { ..Default::default() };
            wfm_object
        }
        pub fn load_file(&mut self, input_file: String) {
            self.file_header = WFMHeader {
                ..Default::default()
            };
            ...
            let mut curve_buf: Vec<i8> = Vec::with_capacity(
                (self.file_header.full_record_length as u32 * self.file_header.num_fastframes)
                    as usize,
            );
            ...
            for entry in &self.file_content.raw_frames {
                self.file_content.scaled_frames.push(
                    (*entry as f64 * self.file_header.voltage_scale)
